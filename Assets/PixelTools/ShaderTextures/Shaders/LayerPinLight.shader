    ...
    Properties
    {
        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                float4 Unity_Blend_PinLight_float4(float4 Base, float4 Blend, float Opacity)
                {
                    float4 check = step(0.5, Blend);
                    float4 result1 = check * max(2.0 * (Base - 0.5), Blend);
                    float4 output = result1 + (1.0 - check) * min(2.0 * Base, Blend);
                ...
                return Unity_Blend_PinLight_float4(base, blend, mask);
                }
                ENDCG
            }
        }
